...
clines%>%filter(param=="centre")%>%group_by(population)%>%
  summarise(mean(median))

clines%>%filter(param=="gradient")%>%group_by(population)%>%
  summarise(mean(median))
...
* get p-values for overlap
* function from dgconstraint: https://github.com/samyeaman/dgconstraint/blob/master/R/allwise_p_chisq.R
...
* make dataframe with loci in rows, populations in columns, where 1=outlier, 0=null
* loop through pairs, trios, and full set

```{r}

clinesArrayGradient<-clines%>%
  filter(param=="gradient")%>%
  dplyr::select(locus,population,outlier)%>%
  mutate(outlier=(as.numeric(as.factor(outlier))-1))%>%
  pivot_wider(names_from=population, values_from=outlier)%>%
  column_to_rownames(var='locus')

outOverlapPval<-data.frame()
...
for(i in 1:nrow(popCombos)){
  ...
  pval1=allwise_p_chisq(clinesArrayGradient%>%dplyr::select(all_of(c(pop1,pop2))))

  outOverlapPval<-rbind(outOverlapPval,
...
for(i in 1:nrow(popCombos3)){
  ...
  pval1=allwise_p_chisq(clinesArrayGradient%>%dplyr::select(all_of(c(pop1,pop2,pop3))))
...
pop1="Alaska"; pop2="Pemberton"; pop3="Hope"; pop4="Washington"
pval1=allwise_p_chisq(clinesArrayGradient)


outOverlapPval<-rbind(outOverlapPval,
                      data.frame(populations=paste(pop1,pop2,pop3,pop4,sep=" x "),
                                 pVal=pval1))
```

```{r}

outOverlap<-outOverlap%>%left_join(outOverlapPval)

outOverlap

write.csv(outOverlap,
          file="C:/Users/Steph/GitHub/thrush_hybrids/genomic_clines/outlier_overlap_Nov24.csv",
          row.names=F)
...
clinesArrayGradientMisexpress<-clinesSummary%>%
  filter(param=="gradient"&ld_dataset=="misexpress")%>%
  mutate(cline_outlier=if_else(countOutlier>1,1,0))%>%
  mutate(misexpress_outlier=if_else(ld_status=="misexpressed",1,0))%>%
  dplyr::select(ChromPos,cline_outlier,misexpress_outlier)%>%
  column_to_rownames(var='ChromPos')

allwise_p_chisq(clinesArrayGradientMisexpress)
sum(clinesArrayGradientMisexpress$cline_outlier)
sum(clinesArrayGradientMisexpress$misexpress_outlier)
sum(clinesArrayGradientMisexpress$cline_outlier==1&
  clinesArrayGradientMisexpress$misexpress_outlier==1)

clinesArrayGradientGWAS<-clinesSummary%>%
  filter(param=="gradient"&ld_dataset=="gwas")%>%
  mutate(cline_outlier=if_else(countOutlier>1,1,0))%>%
  mutate(gwas_outlier=if_else(ld_status=="aphenotype-\nassociated",1,0))%>%
  dplyr::select(ChromPos,cline_outlier,gwas_outlier)%>%
  column_to_rownames(var='ChromPos')
allwise_p_chisq(clinesArrayGradientGWAS)
sum(clinesArrayGradientGWAS$cline_outlier)


any(duplicated(misexpressGenes$ChromPos))
any(duplicated(gwasSNPs$ChromPos))
...
ggsave("C:/Users/Steph/GitHub_data/bgchm_clines/Fig2-revise1.svg",
...
lmm6<-lmer(median~scaffoldLength_Mb+(1|population),data=clines%>%filter(param=="gradient"))
summary(lmm6)
confint(lmm6)
...
lmm6<-lmer(abs(median-0.5)~scaffoldLength_Mb+(1|population),data=clines%>%filter(param=="centre"))
summary(lmm6)
confint(lmm6)
Anova(lmm6)
