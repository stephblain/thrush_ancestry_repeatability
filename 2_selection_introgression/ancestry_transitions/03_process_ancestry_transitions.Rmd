...
tractSummary100kb<-tractSummary100kb%>%filter(ChromPos100kb%in%tractLoci4pops$ChromPos100kb)

tractSummary100kb%>%group_by(ChromPos100kb)%>%
  summarise(n_out=sum(outlierLwr))%>%
  filter(n_out>0)%>%
  nrow()
```
...
write.csv(outOverlap,
          file="C:/Users/Steph/GitHub/thrush_hybrids/ancestry_tracts/outlier_overlap_20241205.csv",
          row.names=F)
...
tractArray<-tractSummary100kb%>%
  dplyr::select(ChromPos100kb,population,outlierLwr)%>%
  pivot_wider(names_from=population, values_from=outlierLwr)%>%
  column_to_rownames(var='ChromPos100kb')

tractArray[is.na(tractArray)]
...
for(i in 1:nrow(popCombos)){
  ...
  pval1=allwise_p_chisq(tractArray%>%dplyr::select(all_of(c(pop1,pop2))))
...
for(i in 1:nrow(popCombos3)){
  ...
  pval1=allwise_p_chisq(tractArray%>%dplyr::select(all_of(c(pop1,pop2,pop3))))
...
pop1="Alaska"; pop2="Pemberton"; pop3="Hope"; pop4="Washington"
pval1=allwise_p_chisq(tractArray)
...
#write.csv(outOverlap,
#        file="C:/Users/Steph/GitHub/thrush_hybrids/ancestry_tracts/outlier_overlap_Dec05.csv",
#          row.names=F)
...
gwasSNPs<-gwasSNPs%>%
  ...
  filter(ChromPos100kb%in%tractLoci4pops$ChromPos100kb)
...
length(unique(gwasTransitionsOutliers$ChromPos100kb))
#nrow(gwasTransitionsOutliers)

n_gwas_windows<-length(gwasSNPs%>%filter(ChromPos100kb%in%tractLoci4pops$ChromPos100kb)%>%
                       pull(ChromPos100kb)%>%unique())
nrow(transitionOutliers)

n_gwas_windows*
  nrow(transitionOutliers)/nrow(tractLoci4pops)

tractArrayGwas<-tractLoci4pops%>%
  dplyr::select(-count)%>%
  left_join(transitionOutliers%>%
              mutate(transitionOutlier=1)%>%
              dplyr::select(ChromPos100kb,transitionOutlier)%>%
              distinct())%>%
  left_join(gwasSNPs%>%
              mutate(gwasLinked=1)%>%
              dplyr::select(ChromPos100kb,gwasLinked)%>%
              distinct())%>%
  column_to_rownames(var='ChromPos100kb')
tractArrayGwas[is.na(tractArrayGwas)]<-0

allwise_p_chisq(tractArrayGwas)
...
#make an array with 1/0 for misexpress location and 4-way outlier location
tractArrayMisexpress<-tractLoci4pops%>%
  dplyr::select(-count)%>%
  left_join(transitionOutliers%>%
              mutate(transitionOutlier=1)%>%
              dplyr::select(ChromPos100kb,transitionOutlier)%>%
              distinct())%>%
  left_join(misexpressTransitionOutliers%>%
  dplyr::select(ChromPos100kbStr)%>%
  distinct()%>%
  mutate(misexpressLinked=1)%>%
  rename(ChromPos100kb=ChromPos100kbStr))%>%
  column_to_rownames(var='ChromPos100kb')
tractArrayMisexpress[is.na(tractArrayMisexpress)]<-0

allwise_p_chisq(tractArrayMisexpress)

#get the fold difference between observed & expected
108/90
```
